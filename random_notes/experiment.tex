...
%
%$$ E(x) = \sum_t\frac{\mu}{2} \Vert D_s - D_m \Vert^2_F $$
%
...
%
%$$ \nabla E(x_i) = tr[(D_s - D_m)^{\mathrm{T}} \cdot \frac{\partial D_s}{\partial x_i}] $$
%
...
%
%Also note that $tr(AB) = tr(BA)$, $tr(A+B) = tr(A)+ tr(B)$, and $tr(A^{\mathrm{T}}) = tr(A)$, then
%
%$$ E(x) = \sum \frac{\mu}{2} \Vert D_s - D_m \vert^2_F = \sum \frac{\mu}{2} \cdot tr((D_s - D_m)\cdot(D_s - D_m)^\mathrm{T}) \newline = \sum \frac{\mu}{2} [ tr(D_s^{\mathrm{T}}D_s - 2D_s^{\mathrm{T}}D_m +D_mD_m^{\mathrm{T}}) 
%
%It's easy to derive:
%
%$tr(D_s^\mathrm{T}D_s) = x^\mathrm{T}G^\mathrm{T}Gx$
%
%$tr(D_s^\mathrm{T} D_m) = x^\mathrm{T}G^\mathrm{T} \cdot vec(D_m)= x^\mathrm{T}G^\mathrm{T} Gx_m$
%
%Thus we have
%
...
%where
%
%$$ A = \mu\sum G^\mathrm{T}G \geq 0 \newline b =\mu \sum G^\mathrm{T} Gx_m \newline c = \frac{\mu}{2} \sum tr(D_mD_m^\mathrm{T}) $$
%
...
%
%Precomputing $(\sum_i S^\mathrm{T}_i S_i)$ can be done via coo\_matrix on $(i,j,k)$ tuples.
